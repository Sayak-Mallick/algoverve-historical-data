...
import { loadSession, signOut } from "./features/session/sessionSlice";
import SignInPage from "./components/Signinpage";
import type { UpstoxProfile as BrokerProfile } from "./types/auth";
...
const paths = {
  ...
  compose: (
    <>
      <path d="M9 4H5.5A1.5 1.5 0 0 0 4 5.5v9A1.5 1.5 0 0 0 5.5 16h9a1.5 1.5 0 0 0 1.5-1.5V11" />
      <path d="m14 3.5 2.5 2.5L10 12.5H7.5V10z" />
    </>
  ),
  link: (
    <>
      <path d="M8.5 11.5a3 3 0 0 0 4.2 0l2.3-2.3a3 3 0 0 0-4.2-4.2l-.8.8" />
      <path d="M11.5 8.5a3 3 0 0 0-4.2 0L5 10.8A3 3 0 0 0 9.2 15l.8-.8" />
    </>
  ),
  layers: (
    <>
      <path d="m10 3.5 6.5 3.25L10 10 3.5 6.75z" />
      <path d="m3.5 10 6.5 3.25L16.5 10" />
  ...
  alert: (
    <>
      <circle cx="10" cy="10" r="7.25" />
      <path d="M10 6.5v4.5" />
      <circle cx="10" cy="13.75" r=".6" fill="currentColor" />
    </>
  ),
  copy: (
    <>
...
const tile =
  "grid shrink-0 place-items-center bg-[#5a2fc2] font-black text-white";

/* ---------- Small components ---------- */
function Dot({ on, className = "" }: { on: boolean; className?: string }) {
  return (
    <span
      className={`size-2 rounded-full ${on ? "bg-presence" : "ring-[1.5px] ring-inset ring-current"} ${className}`}
    />
  );
}
  ...
  return (
    <span
      ...
      className={`inline-flex items-center gap-1 rounded px-2 py-px text-[13px] font-bold ${
        off
          ? "bg-[#f0f0f0] text-ink-3 line-through decoration-ink-3/60"
          : "bg-green-tint text-green-press"
      }`}
    >
      {children}
    </span>
...
function ConfirmDisconnect({
  ...
  return (
    <div
      ...
      onMouseDown={(e) =>
        e.target === e.currentTarget && !pending && onCancel()
      }
    >
      <div
        role="alertdialog"
      ...
      >
        ...
        <p className="mt-3">
          Algoverve will stop receiving data from {brokerName} and your session
          there will end. You can connect again anytime.
        </p>
        {error && (
...
const BROKER_RESULT: Record<string, string> = {
  error: "Upstox connection didn’t finish. Try again.",
  signin_required: "Sign in first, then connect Upstox.",
};
...
function App() {
  ...
  // ?broker=connected | error | signin_required (set by FastAPI after the Upstox redirect)
  const [brokerResult] = useState(() =>
    new URLSearchParams(window.location.search).get("broker"),
  );

  const session = useAppSelector((state) => state.session);
  const { profile, status, error, isAuthenticated } = useAppSelector(
    (state) => state.auth,
  );
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      setLogoutError(
        typeof err === "string" ? err : "Couldn’t disconnect. Try again.",
      );
    } finally {
      setLoggingOut(false);
    }
  };
  const p: BrokerProfile | null = profile?.data ?? null;
  const me = session.user;
  const connectError = brokerResult ? BROKER_RESULT[brokerResult] : undefined;
  ...
  const errorMessage =
    typeof error === "string" ? error : "Unknown error. Try connecting again.";

  if (session.status === "checking") {
    return (
      <div className="grid h-screen place-items-center bg-aubergine-950">
    ...
    <div className="grid h-screen grid-cols-1 grid-rows-[44px_1fr] md:grid-cols-[260px_1fr]">
      ...
      <header className="col-span-full grid grid-cols-[auto_1fr_auto] items-center gap-3 bg-aubergine-950 px-3 md:grid-cols-[1fr_minmax(0,720px)_1fr]">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <div className="relative">
            ...
            >
              {initials(me.name)}
              <span className="absolute -right-[3px] -bottom-[3px] size-2.5 rounded-full bg-presence ring-2 ring-aubergine-950" />
            </button>
            ...
            {menuOpen && (
              <>
                ...
                <div
                  role="menu"
                  className="absolute top-9 right-0 z-50 w-64 rounded-lg border border-line bg-white py-3 text-ink shadow-[0_4px_12px_rgba(0,0,0,0.18)]"
                >
                  <div className="flex items-center gap-3 px-5 pb-3">
                    <span className="grid size-9 place-items-center rounded-md bg-[#e8912d] text-sm font-black text-white">
                      {initials(me.name)}
                    </span>
                  ...
                  <div className="border-t border-line pt-2">
                    <button
                      ...
                      className="w-full px-5 py-1.5 text-left hover:bg-selected hover:text-white"
                    >
                      Sign out of Algoverve
                    </button>
                  </div>
                </div>
              </>
      ...
      <aside className="hidden overflow-y-auto border-r border-white/10 bg-aubergine-900 text-white/70 md:block">
        ...
        <nav className="border-b border-white/10 py-3">
          <a
            aria-current="page"
            className={`${navItem} bg-selected text-white hover:bg-selected`}
          >
            <Icon name="link" size={18} /> Brokers
          </a>
          <a className={navItem}>
            <Icon name="layers" size={18} /> Historical data
          </a>
          <a className={navItem}>
            <Icon name="grid" size={18} /> Instruments
          </a>
        </nav>
        ...
        <div className="py-3">
          ...
          <a className={navItem}>
            <span className={`${tile} size-5 rounded text-[11px]`}>
              {brokerName[0]}
            </span>
            {brokerName}
      ...
      <main className="flex min-w-0 flex-col overflow-hidden bg-white">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {connectError && !isAuthenticated && !isLoading && (
            <div
              role="alert"
            ...
            >
              <Icon name="alert" />
              <p className="text-ink">{connectError}</p>
            </div>
          )}
          ...
          {isLoading && (
            <div className="mx-auto mt-12 flex max-w-[400px] flex-col items-center text-center md:mt-20">
              ...
              <h2 className="mb-1.5 text-lg font-black">
                Checking your Upstox connection
              </h2>
          ...
          {!isLoading && !isAuthenticated && (
            <div className="mx-auto mt-12 flex max-w-[400px] flex-col items-center text-center md:mt-20">
              <span className={`${tile} mb-4 size-14 rounded-xl text-2xl`}>
                U
              </span>
              <h2 className="mb-1.5 text-lg font-black">Connect Upstox</h2>
              <p className="mb-5">
                You’ll sign in on Upstox to authorize Algoverve. Your
                credentials stay with Upstox.
              </p>
              <button
                type="button"
          ...
          {!isLoading && isAuthenticated && p && (
            <div className="mx-auto max-w-[760px]">
              ...
              <div className="flex items-center gap-3 pt-2 pb-4">
                <span className={`${tile} size-12 rounded-lg text-lg`}>
                  {initials(name)}
                </span>
                <div className="min-w-0 flex-1">
                  <h2 className="truncate text-[22px] leading-tight font-black">
                    {name || p.email}
                  </h2>
                  <span className="inline-flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[13px] text-ink-2">
                    <span className="inline-flex items-center gap-1.5">
                      <Dot on={p.is_active} /> {brokerName}{" "}
                      {p.is_active ? "connected" : "account inactive"}
                    </span>
                    <span>{p.user_id}</span>
                  </span>
                </div>
                ...
                >
                  ...
                  <p className="text-[13px]">
                    <strong className="block text-[15px]">
                      Your {brokerName} account is inactive
                    </strong>
                    Orders won’t go through until you reactivate it with{" "}
                    {brokerName}.
                  </p>
                </div>
              )}
              ...
              {tab === "account" && (
                <>
                  ...
                  <Section title="Holdings authorization">
                    <Row
                      label="DDPI"
                      hint="Lets the broker debit shares you sell"
                    >
                    ...
                    {!p.ddpi && !p.poa && (
                      <div className="flex gap-2 bg-surface-2 px-4 py-3 text-[13px] text-ink-2">
                        ...
                        Selling delivery holdings needs a TPIN approval on{" "}
                        {brokerName} each day. Intraday and F&O orders aren’t
                        affected.
                      </div>
                    )}
                  </Section>
              ...
              {tab === "access" && (
                <>
                  ...
                  <Section title="Orders">
                    <Row label="Products">
                      <CodeTags codes={products} names={PRODUCT_NAMES} />
                    </Row>
                    <Row label="Order types">
                      <CodeTags codes={orderTypes} names={ORDER_TYPE_NAMES} />
                    </Row>
                  </Section>
                </>
              )}
